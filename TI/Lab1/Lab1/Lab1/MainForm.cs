...
public partial class MainForm : Form
{
    ...
    private void executeBtn_Click(object sender, EventArgs e)
    {
        string plaintext;
        string result = "";
            if (encypherButton.Checked && decimationButton.Checked)
            {
                plaintext = plainTextBox.Text.ToLower();
                List<int> keyList = TextProcessor.convertToIntList(keyTextbox.Text);
                keyTextbox.Text = string.Concat(keyList);
                if (isKeyIntCorrect())
                {
            ...
            else if ((decipherButton.Checked && decimationButton.Checked ))
            {
                plaintext = plainTextBox.Text.ToLower();
                List<int> keyList = TextProcessor.convertToIntList(keyTextbox.Text);
                keyTextbox.Text = string.Concat(keyList);
